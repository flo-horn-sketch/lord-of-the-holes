      ...
      <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }}>
        <Card className="mb-2 rounded-2xl bg-transparent shadow-none">
          <CardContent className="p-2">
            ...
            <div className={cls("rounded-3xl border p-2 transition-colors", isScorerEntryMode ? "border-sky-400/60 bg-sky-950/25 ring-1 ring-sky-400/35" : "border-amber-400/55 bg-amber-950/20 ring-1 ring-amber-400/25", hasScoreMismatch ? "ring-red-500/45" : "")}>
              {myCurrentPlayer ? <><div className={cls("mb-2 rounded-2xl border px-3 py-2 text-center text-xs font-bold uppercase tracking-[0.16em]", isScorerEntryMode ? "border-sky-400/50 bg-sky-950/60 text-sky-100" : "border-amber-400/50 bg-amber-950/60 text-amber-100")}>{isScorerEntryMode ? `Mithril-Kontrolle · Dein eigener Score (${getPlayerLabel(myCurrentPlayer)})` : `Offizieller Score · Du zählst ${getPlayerLabel(scoredPlayer) || "einen Spieler"}`}</div><div className="mb-2 grid grid-cols-2 gap-2"><button type="button" onClick={() => setScoreEntryMode("player")} className={cls("rounded-2xl px-2 py-3 text-sm font-bold", !isScorerEntryMode ? "bg-amber-600 text-amber-50" : "bg-stone-800 text-amber-100", hasSelectedPlayerScoreMismatch && "ring-1 ring-red-400/60")}><span className="block truncate font-serif text-sm leading-none">{getPlayerLabel(scoredPlayer) || "Spieler"}</span>{hasSelectedPlayerScoreMismatch ? <span className="ml-1">⚠</span> : null}</button><button type="button" onClick={() => setScoreEntryMode("scorer")} className={cls("rounded-xl px-2 py-1.5 text-xs font-bold", isScorerEntryMode ? "bg-amber-600 text-amber-50" : "bg-stone-800 text-amber-100", hasOwnScoreMismatch && "ring-1 ring-red-400/60")}><span className="block truncate font-serif text-sm leading-none">Mein Score</span>{hasOwnScoreMismatch ? <span className="ml-1">⚠</span> : null}</button></div></> : null}
