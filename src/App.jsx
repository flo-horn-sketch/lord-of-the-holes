...
const fallbackCourses = [
  { course_id: "hofhausen", course_name: "Hofhausen vor der Sonne", course_rating: 70.9, slope_rating: 128, par: 72 },
...
function getCourseSettings(id, list = fallbackCourses) {
  ...
  const officialByCourse = {
    goethe: { course_id: "goethe", course_name: "Goethe Kurs", course_rating: 72.0, slope_rating: 131, par: 72 },
    feininger: { course_id: "feininger", course_name: "Feininger Kurs", course_rating: 70.4, slope_rating: 122, par: 71 },
    hofhausen: { course_id: "hofhausen", course_name: "Hofhausen vor der Sonne", course_rating: 70.9, slope_rating: 128, par: 72 },
  };
  const sheetCourse = (list || []).find((item) => String(item?.course_id || "").toLowerCase().trim() === cid);
  return {
    ...(officialByCourse[cid] || officialByCourse.goethe),
    ...(sheetCourse || {}),
    ...(officialByCourse[cid] || {}),
  };
}
...
function getCourseShortName(courseId) {
  ...
  if (normalized === "hofhausen") return "Hofhausen";
  return courseId || "Kurs";
}

function isManualPlayerSelectionCourse(courseId) {
  return String(courseId || "").toLowerCase().trim() === "hofhausen";
}
...
function LordOfTheHolesApp() {
  ...
  const assignedScoredPlayerId = useMemo(() => {
    if (manualPlayerSelectionActive) return "";
    return getAssignedScoredPlayerIdFromDraw(effectiveFlightDraw, displayedActiveRound?.round_id || "", myPlayerId);
  }, [manualPlayerSelectionActive, effectiveFlightDraw, displayedActiveRound?.round_id, myPlayerId]);
  ...
  const isFlightDrawRound = !manualPlayerSelectionActive && ["r1", "r2", "r3"].includes(String(displayedActiveRound?.round_id || ""));
  ...
  useEffect(() => {
    ...
    if (!roundId || !myPlayerId || manualPlayerSelectionActive || !["r1", "r2", "r3"].includes(roundId)) return;
  ...
  }, [displayedActiveRound?.round_id, myPlayerId, manualPlayerSelectionActive, effectiveFlightDraw, scoredPlayerId]);
  useEffect(() => {
    const roundId = String(displayedActiveRound?.round_id || "");
    ...
    if (manualPlayerSelectionActive) {
  ...
  }, [displayedActiveRound?.round_id, myPlayerId, manualPlayerSelectionActive, assignedScoredPlayerId, scoredPlayerId, showSplash, appLocked, lockAdminBypass, scoredPlayerByRound, scoreablePlayers, isFlightDrawRound]);
  ...
  async function saveFullSetup() {
    ...
    const nextAllPlayers = allPlayers.map((p) => {
      ...
      return { ...nextPlayer, course_hcp_goethe: getCourseHandicap(player, displayCourseId, coursesForUse)coursesForUse), course_hgetCourseHandicap(player, displayCourseId, coursesForUse)er, "feininger", coursesgetCourseHandicap(player, displayCourseId, coursesForUse)Handicap(nextPlayer, "hofhausen", coursesForUse) };
    });
  ...
  function renderLeaderboardView() {
    ...
    const tableCourse = coursesForUse.find((course) => String(course.course_id) === String(tableCourseId));
  ...
  function renderArchiveView() {
    ...
    const archiveCourse = coursesForUse.find((course) => String(course.course_id) === String(archiveCourseId));
