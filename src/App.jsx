  ...
  function renderScoreView() {
    return (
      <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }}>
        <Card className="mb-2 rounded-2xl border-amber-700/40 bg-[#20170f]/82 shadow-xl backdrop-blur-sm">
          <CardContent className="p-2">
            <div className={cls("mb-2 rounded-xl border bg-black/25 p-1.5", hasScoreMismatch ? "border-red-500/60" : "border-amber-700/30")}><div className="flex items-center justify-between gap-2"><div><div className="text-[10px] uppercase tracking-[0.18em] text-amber-300/75">Aktuell gespielt</div><div className="font-serif text-base leading-tight text-amber-200">{displayedActiveRound?.round_name || "Runde 1"}</div><div className="text-[10px] text-amber-100/65">{activeCourse?.course_name || "Kein Kurs ausgewählt"}</div></div>{hasScoreMismatch && <div className="rounded-full border border-red-400/50 bg-red-950/50 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wide text-red-100"},{>Abweichung</div>}</div></div>
            <div className="mb-2 grid grid-cols-[1fr_auto] items-center gap-2"><div><p className="text-[10px] uppercase tracking-[0.18em] text-amber-300/75">Aktives Loch</p><h2 className="font-serif text-3xl font-black leading-none text-amber-200">{activeHole}</h2></div><div className="grid grid-cols-3 gap-1 rounded-xl border border-amber-700/50 bg-black/30 p-1.5 text-center text-[11px] text-amber-50"><div><div className="text-amber-100/70">Par</div><b className="text-amber-200">{activeHoleData.par}</b></div><div><div className="text-amber-100/70">HCP</div><b className="text-amber-200">{activeHoleData.hcp}</b></div><div><div className="text-amber-100/70">m</div><b className="text-amber-200">{activeHoleData.meters}</b></div></div></div>
            {myCurrentStats ? <div className="mb-2 w-full rounded-xl border border-amber-700/30 bg-black/25 p-1.5 text-left"><div className="mb-1 flex items-center justify-between gap-2"><div className="text-[10px] uppercase tracking-[0.18em] text-amber-300/75">Mein Stand</div><div className="font-serif text-xs text-amber-200">{getPlayerLabel(myCurrentStats)}</div></div><div className="grid grid-cols-2 gap-2 text-center text-sm"><button type="button" onClick={() => setStandingsPopup("hcpAdjusted")} className="rounded-xl bg-amber-50/5 p-1.5 text-amber-50 transition active:scale-[0.99]"},{><div className="text-amber-100">Strokes</div><b className="text-lg text-amber-200">{myCurrentStats.played ? myCurrentStats.hcpAdjustedTotal : "–"}</b><div className="mt-0.5 text-[11px] text-amber-100/70">Tatsächlich {myCurrentStats.played ? myCurrentStats.total : "–"} · Platz {myHcpAdjustedStrokeRank || "–"}</div></button><button type="button" onClick={() => setStandingsPopup("netStableford")} className="rounded-xl bg-amber-50/5 p-1.5 text-amber-50 transition active:scale-[0.99]"},{><div className="text-amber-100">Netto Stbl</div><b className="text-lg text-amber-200">{myCurrentStats.netStableford}</b><div className="mt-0.5 text-[11px] text-amber-100/70">SpV {Number(myCurrentStats.course_hcp || 0)} · Platz {myNetStablefordRank || "–"}</div></button></div></div> : <div className="mb-3 rounded-xl border border-amber-700/30 bg-black/20 p-2 text-xs text-amber-100/75">Unter Einstellungen kannst du festlegen, wer du bist. Danach erscheint hier dein aktueller Score.</div>}
