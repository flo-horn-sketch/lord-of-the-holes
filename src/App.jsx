...
const fallbackPlayers = [
  {
    id: "florian",
    character_name: "Florian",
    display_name: "Florian",
    sort_order: 1,
    course_hcp_goethe: 0,
    course_hcp_feininger: 0,
  },
  {
    id: "mucky",
    character_name: "Mucky",
    display_name: "Mucky",
    sort_order: 2,
    course_hcp_goethe: 0,
    course_hcp_feininger: 0,
...
function normalizeBoolean(value) {
  return (
    ...
    String(value).toLowerCase().trim() === "ja"
  );
}
...
function getCourseHandicap(player, courseId = "goethe") {
  ...
  if (normalizedCourseId === "feininger") {
    return Number(player?.course_hcp_feininger ?? 0);
  }
...
function getPuttBuckets(playerScores) {
  const threePutts = playerScores.filter(
    (s) => normalizeBoolean(s.over_two_putts) && Number(s.putts_count) === 3
  ).length;

  const fourPlusPutts = playerScores.filter(
    (s) => normalizeBoolean(s.over_two_putts) && Number(s.putts_count) >= 4
  ).length;

  return {
    threePutts,
    fourPlusPutts,
    overTwoPutts: threePutts + fourPlusPutts,
  };
}

function getRoundCourse(round, courses) {
  return courses.find(
    (course) => String(course.course_id) === String(round?.course_id)
  );
}
...
function getRoundPlayers(roundId, allPlayers, roundPlayers) {
  const rowsForRound = Array.isArray(roundPlayers)
    ? roundPlayers.filter(
        (rp) => String(rp.round_id).trim() === String(roundId).trim()
      )
    : [];
...
function buildPlayerStats(players, holes, scores) {
  return players.map((p) => {
    const playerScores = scores.filter(
      (s) => s.player_id === p.id && s.strokes !== "" && s.strokes != null
    );
    ...
    const parPlayed = playerScores.reduce((sum, s) => {
      const hole = holes.find(
        (h) => Number(h.hole_number) === Number(s.hole_number)
      );
      return sum + Number(hole?.par || 0);
    }, 0);

    const { threePutts, fourPlusPutts, overTwoPutts } =
      getPuttBuckets(playerScores);
...
function sortStableford(stats, fieldName) {
  return [...stats].sort((a, b) => {
    ...
    return (
      Number(b[fieldName] || 0) - Number(a[fieldName] || 0) ||
      b.played - a.played ||
      Number(a.sort_order || 0) - Number(b.sort_order || 0)
    );
  });
...
function sortHcpAdjustedStrokePlay(stats) {
  return [...stats].sort((a, b) => {
    ...
    return (
      Number(a.hcpAdjustedToPar || 0) - Number(b.hcpAdjustedToPar || 0) ||
      Number(a.hcpAdjustedTotal || 0) - Number(b.hcpAdjustedTotal || 0) ||
      b.played - a.played ||
      Number(a.sort_order || 0) - Number(b.sort_order || 0)
    );
  });
...
function sortPuttPenalties(stats) {
  return [...stats].sort(
    (a, b) =>
      Number(b.puttPenaltyEuro || 0) - Number(a.puttPenaltyEuro || 0) ||
      Number(a.sort_order || 0) - Number(b.sort_order || 0)
  );
}
...
function getFinalRound(rounds) {
  return (
    (rounds?.length ? rounds : fallbackRounds).find(
      (round) => String(round.round_id) === "r4"
    ) ||
    (rounds?.length ? rounds : fallbackRounds).find(
      (round) => String(round.stage) === "final"
    ) ||
    fallbackRounds[3]
  );
}
  ...
  return players
    .map((player) => {
      const roundResults = qualificationRounds.map((round) => {
        ...
        const playedScores = roundScores.filter(
          (score) => score.strokes !== "" && score.strokes != null
        );

        const netStableford = playedScores.reduce((sum, score) => {
          const hole = roundHoles.find(
            (h) => Number(h.hole_number) === Number(score.hole_number)
          );
        ...
        return {
          ...
          points: netStableford,
          played: playedScores.length,
        };
      });

      const playedResults = roundResults.filter((result) => result.played > 0);
      const sortedPlayed = [...playedResults].sort(
    ...
    .sort(
      (a, b) =>
        Number(b.totalBestTwo || 0) - Number(a.totalBestTwo || 0) ||
        Number(b.roundsPlayed || 0) - Number(a.roundsPlayed || 0) ||
        Number(a.sort_order || 0) - Number(b.sort_order || 0)
    );
}
  ...
  return players
    .map((player) => {
      const roundResults = puttRounds.map((round) => {
        ...
        return {
          round_id: round.round_id,
          round_name: round.round_name,
          threePutts: buckets.threePutts,
          fourPlusPutts: buckets.fourPlusPutts,
          amount: buckets.threePutts * 2 + buckets.fourPlusPutts * 4,
        };
      });
      ...
      return {
        ...
        totalFourPlusPutts: roundResults.reduce(
          (sum, r) => sum + r.fourPlusPutts,
          0
        ),
    ...
    .sort(
      (a, b) =>
        ...
        Number(b.totalFourPlusPutts || 0) - Number(a.totalFourPlusPutts || 0) ||
        Number(a.sort_order || 0) - Number(b.sort_order || 0)
    );
}

function buildFinalNetStandings(players, rounds, holes, scores) {
  const qualificationStandings = buildTournamentNetStandings(
    players,
    rounds,
    holes,
    scores
  ...
  const withFinalScores = qualificationStandings.map(
    (player, qualificationIndex) => {
      ...
      const playedFinalScores = finalScores.filter(
        (score) => score.strokes !== "" && score.strokes != null
      );

      const finalNetStableford = playedFinalScores.reduce((sum, score) => {
        const hole = finalHoles.find(
          (h) => Number(h.hole_number) === Number(score.hole_number)
        );
  ...
  const championshipGroup = withFinalScores
    ...
    .sort(
      (a, b) =>
        Number(b.finalNetStableford || 0) - Number(a.finalNetStableford || 0) ||
        Number(a.qualificationRank || 0) - Number(b.qualificationRank || 0)
    )
...
function buildScorecardRows(player, round, holes, scores) {
  ...
  const roundScores = scores.filter(
    (s) =>
      String(s.round_id) === String(round?.round_id) &&
      String(s.player_id) === String(player?.id)
  );

  return roundHoles.map((hole) => {
    const score = roundScores.find(
      (s) => Number(s.hole_number) === Number(hole.hole_number)
    );

    const strokes =
      score?.strokes === "" || score?.strokes == null
        ? null
...
function summarizeScorecard(rows) {
  ...
  return {
    ...
    totalStrokes: playedRows.reduce(
      (sum, row) => sum + Number(row.strokes || 0),
      0
    ),
    toPar:
      playedRows.reduce((sum, row) => sum + Number(row.strokes || 0), 0) -
      playedRows.reduce((sum, row) => sum + Number(row.hole.par || 0), 0),
    netStableford: playedRows.reduce(
      (sum, row) => sum + Number(row.netStableford || 0),
      0
    ),
    grossStableford: playedRows.reduce(
      (sum, row) => sum + Number(row.grossStableford || 0),
      0
    ),
...
function runSelfTests() {
  ...
  assert(
    "url string is closed",
    typeof GOOGLE_SHEETS_API_URL === "string" &&
      GOOGLE_SHEETS_API_URL.endsWith("/exec")
  );
  ...
  assert(
    "picked up score gives zero net points",
    getStablefordPoints(
      getPickedUpStrokes({ course_hcp_goethe: 5 }, { par: 4, hcp: 5 }, "goethe"),
      4,
      getShotsOnHole(5, 5)
    ) === 0
  );
  ...
  assert(
    "current handicap helper works",
    getPlayerForCourse(
      { ...fallbackPlayers[0], course_hcp_goethe: 7 },
      "goethe"
    )?.course_hcp === 7
  );
  ...
  assert(
    "putt money calculates",
    stats.find((p) => p.id === "florian")?.puttPenaltyEuro === 4
  );
  ...
  if (failures.length) {
    console.warn("Lord of the Holes self-tests failed:", failures);
  }
}
...
function LeaderboardTable({ title, players, columns }) {
  return (
    <div className="mb-3 overflow-hidden rounded-2xl border border-amber-700/30 bg-black/20">
      <div className="border-b border-amber-700/30 bg-amber-500/10 px-2.5 py-1.5 font-serif text-lg text-amber-200">
        {title}
      </div>
        ...
        <table className="w-full min-w-[360px] border-collapse text-sm text-amber-50">
          <thead>
            <tr className="text-left text-xs uppercase tracking-wider text-amber-100">
              ...
              {columns.map((column) => (
                <th key={column.label} className="px-2.5 py-1.5 text-right">
                  {column.label}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {players.map((p, index) => (
              <tr key={p.id} className="border-t border-amber-700/20">
                <td className="px-2.5 py-1.5 text-amber-200/75">
                  {index + 1}
                </td>
                ...
                {columns.map((column) => (
                  <td
                    ...
                    className={cls(
                      ...
                      column.emphasize && "font-serif text-lg text-amber-300"
                    )}
                  >
                    {column.render(p)}
                  </td>
...
function TournamentStandings({ players, rounds, holes, scores, activeRoundId = "" }) {
  const standings = useMemo(
    () => buildTournamentNetStandings(players, rounds, holes, scores),
    [players, rounds, holes, scores]
  );

  const finalStandings = useMemo(
    () => buildFinalNetStandings(players, rounds, holes, scores),
    [players, rounds, holes, scores]
  );

  const puttStandings = useMemo(
    () => buildTournamentPuttStandings(players, rounds, scores),
  ...
  return (
    <Card className="mb-3 rounded-2xl border-amber-700/40 bg-[#20170f]/90 shadow-xl">
      <CardContent className="p-3">
        <div className="mb-3">
          <p className="text-xs uppercase tracking-[0.2em] text-amber-300/75">
            Turnier
          </p>
          <h2 className="font-serif text-lg text-amber-200">
            {isFinalActive ? "Finalwertung Netto" : "Gesamtwertung Netto"}
          </h2>
                  ...
                  <React.Fragment key={player.id}>
                    {index === 3 ? (
                      <tr>
                        <td
                          colSpan={6}
                          className="border-y-2 border-amber-400/70 bg-amber-500/10 px-2.5 py-2 text-center text-xs font-bold uppercase tracking-[0.2em] text-amber-200"
                        >
                          Platzierungsgruppe · Plätze 4–6
                        </td>
                      </tr>
                    ) : null}
                    ...
                    <tr
                      className={cls(
                        "border-t border-amber-700/20",
                        index < 3 && "bg-emerald-500/5"
                      )}
                    >
                      <td className="px-2.5 py-1.5 font-serif text-lg font-bold text-amber-300">
                        {player.finalRank}
                      </td>
                      <td className="px-2.5 py-1.5 font-semibold text-amber-100">
                        {player.character_name}
                      </td>
                      <td className="px-2.5 py-1.5 text-right text-amber-100/75">
                        {player.qualificationRank}
                      </td>
                      <td className="px-2.5 py-1.5 text-right font-serif text-xl font-bold text-amber-300">
                        {player.finalPlayed ? player.finalNetStableford : "–"}
                      </td>
                      <td className="px-2.5 py-1.5 text-right text-amber-100">
                        {player.finalPlayed}/18
                      </td>
                      <td className="px-2.5 py-1.5 text-right text-amber-100/75">
                        {player.finalGroup === "championship" ? "1–3" : "4–6"}
                      </td>
                    </tr>
                  </React.Fragment>
                ))}
              ...
              <thead>
                <tr className="text-left text-xs uppercase tracking-wider text-amber-100">
                  ...
                  {qualificationRounds.map((round) => (
                    <th key={round.round_id} className="px-2.5 py-1.5 text-right">
                      {round.round_name}
                    </th>
                  ))}
                ...
                {standings.map((player, index) => (
                  <React.Fragment key={player.id}>
                    {index === 3 ? (
                      <tr>
                        <td
                          colSpan={qualificationRounds.length + 5}
                          className="border-y-2 border-amber-400/70 bg-amber-500/10 px-2.5 py-2 text-center text-xs font-bold uppercase tracking-[0.2em] text-amber-200"
                        >
                          Cut-Linie · Top 3 spielen den Finaltag
                        </td>
                      </tr>
                    ) : null}
                    ...
                    >
                      ...
                      <td className="px-2.5 py-1.5 font-semibold text-amber-100">
                        {player.character_name}
                        {index < 3 ? (
                          <span className="ml-2 rounded-full bg-emerald-500/20 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-emerald-200">
                            Final
                          </span>
                        ) : null}
                      </td>
                      ...
                      {qualificationRounds.map((round) => {
                        const result = player.roundResults.find(
                          (item) => item.round_id === round.round_id
                        );
                        const isCounted = player.countedRoundIds.includes(
                          round.round_id
                        );
                        ...
                        return (
                          <td
                            ...
                            className={cls(
                              "px-2.5 py-1.5 text-right",
                              isCounted && "font-bold text-amber-300",
                              isDropped && "text-amber-100/50 line-through"
                            )}
                          >
                            {result?.played ? result.points : "–"}
                          </td>
                        );
                      })}

                      <td className="px-2.5 py-1.5 text-right text-amber-100/70">
                        {player.droppedRoundId
                          ? player.roundResults.find(
                              (item) => item.round_id === player.droppedRoundId
                            )?.points ?? "–"
                          : "–"}
                      </td>
        ...
        <div className="mt-6 mb-4">
          ...
          <h3 className="font-serif text-lg text-amber-200">
            Gesamt über Runde 1–4
          </h3>
          <p className="mt-1 text-sm text-amber-100/70">
            3 Putts zählen 2 €, 4+ Putts zählen 4 €.
          </p>
        </div>
                ...
                <tr key={player.id} className="border-t border-amber-700/20">
                  ...
                  {puttKasseRounds.map((round) => {
                    ...
                    return (
                      <td
                        key={round.round_id}
                        className="px-2.5 py-1.5 text-right text-amber-100"
                      >
                  ...
                  <td className="px-2.5 py-1.5 text-right text-amber-100">
                    {player.totalThreePutts} × 2 €
                  </td>
                  <td className="px-2.5 py-1.5 text-right text-amber-100">
                    {player.totalFourPlusPutts} × 4 €
                  </td>
                  <td className="px-2.5 py-1.5 text-right font-serif text-xl font-bold text-amber-300">
                    {player.totalAmount} €
                  </td>
...
function ScorecardArchive({
  ...
  const [selectedRoundId, setSelectedRoundId] = useState(
    availableRounds[0]?.round_id || ""
  );

  const rawSelectedRound =
    availableRounds.find((round) => String(round.round_id) === String(selectedRoundId)) ||
    availableRounds[0] ||
    null;
  ...
  const selectedRound = rawSelectedRound
    ? {
        ...rawSelectedRound,
        course_id:
          rawSelectedRound.course_id ||
          (String(rawSelectedRound.round_id) === String(selectedRoundId)
            ? selectedCourseId
            : ""),
      }
    : null;
  ...
  useEffect(() => {
    if (!availableRounds.some((round) => String(round.round_id) === String(selectedRoundId))) {
      setSelectedRoundId(availableRounds[0]?.round_id || "");
    }
  ...
  const selectedPlayer =
    eligiblePlayers.find((p) => String(p.id) === String(selectedPlayerId)) ||
    eligiblePlayers[0] ||
    null;

  const rows = useMemo(
    () => buildScorecardRows(selectedPlayer, selectedRound, holes, scores),
    [selectedPlayer, selectedRound, holes, scores]
  );
  ...
  const archiveRoundLabel = (round) => {
    const effectiveRound = {
      ...
      course_id:
        round.course_id ||
        (String(round.round_id) === String(selectedRoundId)
          ? selectedCourseId
          : ""),
    };
      ...
      <CardContent className="p-3">
        ...
        <div className="mb-3 grid gap-2 rounded-xl border border-amber-700/30 bg-black/25 p-2.5">
          ...
          <select
            ...
            className="w-full rounded-2xl border border-amber-700/40 bg-stone-950 p-2.5 text-amber-50"
          >
            {availableRounds.map((round) => (
              <option key={round.round_id} value={round.round_id}>
                {archiveRoundLabel(round)}
              </option>
            ))}
          </select>
        ...
        {!selectedRound?.course_id ? (
          <div className="rounded-2xl border border-amber-700/30 bg-amber-950/40 p-4 text-sm text-amber-100">
            Für diese Runde ist noch kein Kurs in den Einstellungen gespeichert.
          </div>
        ) : !selectedPlayer ? (
          <div className="rounded-2xl border border-amber-700/30 bg-amber-950/40 p-4 text-sm text-amber-100">
            Für diese Runde ist kein Spieler ausgewählt.
          </div>
        ) : (
          <>
            <div className="mb-3 grid grid-cols-2 gap-2 text-center text-sm">
              {[
                ...
                <div key={label} className="rounded-2xl bg-amber-50/5 p-2.5 text-amber-50">
                  <div className="text-amber-100">{label}</div>
                  <b className="text-amber-200">{value}</b>
                </div>
              ))}
            </div>
            ...
            <div className="overflow-x-auto rounded-2xl border border-amber-700/30 bg-black/20 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              <table className="w-full min-w-[760px] border-collapse text-sm text-amber-50">
                <thead>
                  <tr className="text-left text-xs uppercase tracking-wider text-amber-100">
                    <th className="sticky left-0 z-10 bg-[#20170f] px-2.5 py-1.5">
                      Loch
                    </th>
                ...
                <tbody>
                  {rows.map((row) => (
                    <tr key={row.hole.hole_number} className="border-t border-amber-700/20">
                      <td className="sticky left-0 z-10 bg-[#20170f] px-2.5 py-1.5 font-semibold text-amber-100">
                        {row.hole.hole_number}
                      </td>
                      ...
                      <td className="px-2.5 py-1.5 text-right font-semibold text-amber-200">
                        {row.strokes == null
                          ? "–"
                          : row.isPickedUp
                            ? `${row.strokes} gestr.`
                      ...
                      <td className="px-2.5 py-1.5 text-right">
                        {row.toPar == null ? "–" : formatToPar(row.toPar)}
                      </td>
                      <td className="px-2.5 py-1.5 text-right">
                        {row.strokes == null ? "–" : row.netStableford}
                      </td>
                      <td className="px-2.5 py-1.5 text-right">
                        {row.strokes == null ? "–" : row.grossStableford}
                      </td>
                ...
                <tfoot>
                  <tr className="border-t border-amber-500/40 bg-amber-500/10 font-bold text-amber-100">
                    <td className="sticky left-0 z-10 bg-[#2a2117] px-2.5 py-1.5">
                      Total
                    </td>
                    <td className="px-2.5 py-1.5 text-right">–</td>
                    <td className="px-2.5 py-1.5 text-right">
                      {rows.reduce((sum, row) => sum + Number(row.hole.par || 0), 0)}
                    </td>
                    <td className="px-2.5 py-1.5 text-right">–</td>
                    <td className="px-2.5 py-1.5 text-right text-amber-300">
                      {summary.played ? summary.totalStrokes : "–"}
                    </td>
                    <td className="px-2.5 py-1.5 text-right">
                      {summary.played ? formatToPar(summary.toPar) : "–"}
                    </td>
                    <td className="px-2.5 py-1.5 text-right text-amber-300">
                      {summary.netStableford}
                    </td>
                    <td className="px-2.5 py-1.5 text-right text-amber-300">
                      {summary.grossStableford}
                    </td>
                    <td className="px-2.5 py-1.5 text-right">
                      3× {summary.threePutts} · 4+× {summary.fourPlusPutts}
                    </td>
                  </tr>
                </tfoot>
              </table>
...
async function callSheetApi(payload) {
  ...
  if (data && data.ok === false) {
    throw new Error(data.error || "Datenbank meldet einen Fehler.");
  }
...
export default function LordOfTheHolesPWA() {
  ...
  const [myPlayerId, setMyPlayerId] = useState(() =>
    readLocalJson("lordOfTheHoles.myPlayerId", "")
  );
  ...
  const displayedActiveRound =
    (selectedActiveRoundId &&
      (rounds.length ? rounds : fallbackRounds).find(
        (round) => String(round.round_id) === String(selectedActiveRoundId)
      )) ||
    activeRound ||
  ...
  const activeCourse = (courses.length ? courses : fallbackCourses).find(
    (course) => String(course.course_id) === String(displayCourseId)
  );
  ...
  const scoreablePlayers = useMemo(() => {
    const filteredPlayers = myPlayerId
      ? visiblePlayers.filter((p) => String(p.id) !== String(myPlayerId))
      : visiblePlayers;
  ...
  const playersWithCurrentHandicaps = useMemo(
    () => getPlayersForCourse(visiblePlayers, displayCourseId),
    [visiblePlayers, displayCourseId]
  );

  const activeHoleData =
    holes.find((h) => Number(h.hole_number) === activeHole) ||
    holes[activeHole - 1] ||
    fallbackHoles[activeHole - 1];
  ...
  const pickedUpStrokes = getPickedUpStrokes(
    scoredPlayer,
    activeHoleData,
    displayCourseId
  );

  const myCurrentPlayer = myPlayerId
    ? getPlayerForCourse(
        visiblePlayers.find((player) => String(player.id) === String(myPlayerId)),
        displayCourseId
      )
    : null;

  const myShotsOnActiveHole = getShotsOnHole(
    myCurrentPlayer?.course_hcp,
    activeHoleData?.hcp
  );

  const scoredPlayerShotsOnActiveHole = getShotsOnHole(
    scoredPlayer?.course_hcp,
    activeHoleData?.hcp
  );
  ...
  const currentScore = useMemo(
    () =>
      scores.find(
        (s) =>
          String(s.round_id || "") === String(displayedActiveRound?.round_id || "r1") &&
          s.player_id === scoredPlayerId &&
          Number(s.hole_number) === activeHole
      ) || {
        strokes: "",
        picked_up: false,
        over_two_putts: false,
  ...
  const playerStats = useMemo(
    () => buildPlayerStats(playersWithCurrentHandicaps, holes, scores),
    [playersWithCurrentHandicaps, holes, scores]
  );

  const myCurrentStats = useMemo(
    () =>
      myPlayerId
        ? playerStats.find((player) => String(player.id) === String(myPlayerId)) ||
          null
        : null,
    [playerStats, myPlayerId]
  );

  const strokePlayLeaderboard = useMemo(() => sortStrokePlay(playerStats), [playerStats]);
  const netStablefordLeaderboard = useMemo(
    () => sortStableford(playerStats, "netStableford"),
    [playerStats]
  );
  const grossStablefordLeaderboard = useMemo(
    () => sortStableford(playerStats, "grossStableford"),
    [playerStats]
  );
  const hcpAdjustedStrokeLeaderboard = useMemo(
    () => sortHcpAdjustedStrokePlay(playerStats),
    [playerStats]
  );
  const puttPenaltyLeaderboard = useMemo(
    () => sortPuttPenalties(playerStats),
    [playerStats]
  );

  const myStrokeRank = useMemo(() => {
    const index = strokePlayLeaderboard.findIndex(
      (player) => String(player.id) === String(myPlayerId)
    );
  ...
  const myNetStablefordRank = useMemo(() => {
    const index = netStablefordLeaderboard.findIndex(
      (player) => String(player.id) === String(myPlayerId)
    );
  ...
  const quickScores = useMemo(() => {
    ...
    return Array.from(
      new Set([par - 1, par, par + 1, par + 2, par + 3, par + 4].filter((v) => v > 0))
    );
  }, [activeHoleData?.par]);

  useEffect(() => {
    if (!scoreablePlayers.some((p) => String(p.id) === String(scoredPlayerId))) {
  ...
  useEffect(() => {
    ...
    const selectedRoundScores = allScores.filter(
      (score) => String(score.round_id || "") === String(selectedActiveRoundId)
    );
  ...
  function applyPlayers(nextActivePlayers, nextAllPlayers = nextActivePlayers) {
    ...
    nextAllPlayers.forEach((player) => {
      nextHandicaps[getCourseHcpKey(player.id, "goethe")] = String(
        player.course_hcp_goethe ?? 0
      );

      nextHandicaps[getCourseHcpKey(player.id, "feininger")] = String(
        player.course_hcp_feininger ?? 0
      );
    });
  ...
  async function loadData({ silent = false } = {}) {
    ...
    try {
      ...
      const nextActiveRound =
        data.activeRound ||
        nextRounds.find((round) => String(round.status).toLowerCase() === "active") ||
        nextRounds[0] ||
        fallbackRounds[0];

      const nextActivePlayers = data.activePlayers?.length
        ? data.activePlayers
        : getRoundPlayers(nextActiveRound?.round_id, nextAllPlayers, data.roundPlayers || []);
  ...
  function optimisticUpdate(patch) {
    ...
    setScores((current) => {
      const exists = current.some(
        (s) =>
          String(s.round_id) === String(next.round_id) &&
          String(s.player_id) === String(next.player_id) &&
          Number(s.hole_number) === Number(next.hole_number)
      );
  ...
  async function saveFullSetup() {
    ...
    const nextAllPlayers = allPlayers.map((p) => {
      ...
      return {
        ...p,
        course_hcp_goethe:
          goetheValue !== "" ? Number(goetheValue) : Number(p.course_hcp_goethe ?? 0),
        course_hcp_feininger:
          feiningerValue !== ""
            ? Number(feiningerValue)
            : Number(p.course_hcp_feininger ?? 0),
      };
    });
    ...
    try {
      ...
      setRounds((currentRounds) =>
        (currentRounds.length ? currentRounds : fallbackRounds).map((round) => ({
          ...round,
          course_id:
            String(round.round_id) === String(selectedActiveRoundId)
              ? selectedCourseId || round.course_id || ""
              : round.course_id,
          status:
            String(round.round_id) === String(selectedActiveRoundId)
              ? "active"
  ...
  async function saveRoundToArchive() {
    const roundToSave = displayedActiveRound || {
      round_id: "r1",
      round_name: "Runde 1",
    };
    ...
    try {
      ...
      setError("");
      setRoundSavedMessage(
        `${roundToSave.round_name || "Runde"} wurde gespeichert und ins Archiv übernommen.`
      );
      await loadData({ silent: true });
      setView("archive");
    } catch (err) {
      ...
      <main className="relative mx-auto max-w-md px-3 py-3">
        ...
        >
          <div className="mb-1 flex items-center justify-between gap-2">
            <div className="inline-flex items-center gap-2 rounded-full border border-amber-500/30 bg-black/35 px-2.5 py-1 text-[10px] uppercase tracking-[0.18em] text-amber-200">
              Weimarer Land
            </div>
            ...
            <button
              ...
              className="rounded-xl border border-amber-700/40 bg-black/25 px-3 py-1.5 text-lg leading-none text-amber-100"
          ...
          <div className="relative text-center">
            <h1 className="font-serif text-3xl font-black tracking-wide text-amber-300 drop-shadow">
              Lord of the Holes
            </h1>

            <p className="mt-0.5 text-xs text-amber-100/75">
              {mainMenu === "current"
                ? displayedActiveRound?.round_name || "Aktive Runde"
                : mainMenu === "tournament"
                  ? "Turnier"
                  : mainMenu === "archive"
                    ? "Archiv"
              ...
              <div className="absolute right-0 top-[58px] z-30 w-64 overflow-hidden rounded-2xl border border-amber-700/40 bg-stone-950/95 text-left shadow-2xl shadow-black/70 backdrop-blur">
                {[
                  ...
                  <button
                    ...
                    className={cls(
                      "block w-full border-b border-amber-700/20 px-4 py-4 text-left text-sm last:border-b-0",
                      mainMenu === value
          ...
          <div className="mt-2 flex justify-center">
            <div className="inline-flex items-center gap-2 rounded-full border border-amber-700/40 bg-black/35 px-2.5 py-1 text-[11px] text-amber-100/75">
              <Icon
                size={14}
                className={
                  connectionStatus === "online" ? "text-emerald-300" : "text-red-300"
                }
              >
        ...
        {error ? (
          <Card className="mb-3 rounded-2xl border-amber-700/40 bg-amber-950/50">
        ...
        {roundSavedMessage ? (
          <Card className="mb-3 rounded-2xl border-emerald-700/40 bg-emerald-950/40">
            <CardContent className="p-3 text-sm text-emerald-100">
              {roundSavedMessage}
            </CardContent>
          </Card>
        ) : null}
        ...
        {mainMenu === "current" ? (
          <div className="mb-3 grid grid-cols-2 gap-2">
            <Button
              ...
              className={cls(
                "rounded-xl px-1 py-2.5 text-sm font-bold",
                view === "score"
                  ? "bg-amber-600 text-amber-50"
        ...
        {loading ? (
          <Card className="rounded-2xl border-amber-700/40 bg-[#20170f]/90 shadow-xl">
            <CardContent className="flex items-center gap-2 p-3 text-amber-100">
              <Icon spin>⟳</Icon> Lade Datenbank ...
            </CardContent>
          </Card>
        ) : view === "tournament" ? (
          <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }}>
            <TournamentStandings
              ...
              holes={allHoles}
              scores={allScores}
              activeRoundId={displayedActiveRound?.round_id}
            />
          </motion.section>
        ) : view === "admin" ? (
          <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }}>
              ...
              <CardContent className="p-3">
                <div className="mb-3">
                  ...
                  <p className="mt-1 text-sm text-amber-100/65">
                    Aktive Runde und Spielvorgaben sind sichtbar, aber erst nach
                    Passworteingabe bearbeitbar.
                  </p>
                </div>
                ...
                {!isAdminUnlocked ? (
                  <div className="mb-3 rounded-2xl border border-amber-700/30 bg-black/25 p-2.5">
                    ...
                    <input
                      ...
                      className="mb-3 w-full rounded-2xl border border-amber-700/40 bg-stone-950 p-2.5 text-amber-50 placeholder:text-amber-100/30"
                    />
                    <Button
                      onClick={() => {
                      ...
                      className="w-full rounded-2xl bg-amber-600 text-amber-50"
                    >
                      Admin entsperren
                    </Button>
                  </div>
                ) : (
                  <div className="mb-3 rounded-2xl border border-emerald-700/30 bg-emerald-950/30 p-3 text-sm text-emerald-100">
                    Admin entsperrt. Änderungen können gespeichert werden.
                  </div>
                )}
                ...
                <div className="mb-3 rounded-2xl border border-amber-700/30 bg-black/25 p-2.5">
                  ...
                  <select
                    value={selectedActiveRoundId}
                    onChange={(e) => {
                      setAdminEditing(true);
                      setSelectedActiveRoundId(e.target.value);
                    }}
                    disabled={!isAdminUnlocked}
                    className="w-full rounded-2xl border border-amber-700/40 bg-stone-950 p-2.5 text-amber-50 disabled:opacity-60"
                  >
                ...
                <div className="space-y-2">
                  {allPlayers.map((p) => {
                    ...
                    return (
                      <div
                        key={p.id}
                        className="rounded-xl border border-amber-700/30 bg-black/25 p-2.5"
                      >
                        <div className="mb-2 font-semibold text-amber-100">
                          {p.character_name}
                          <div className="text-xs font-normal text-amber-100/70">
                            2 Kurs-Spielvorgaben
                          </div>
                        </div>

                        <div className="grid grid-cols-1 gap-2">
                          <div className="rounded-xl border border-amber-700/20 bg-black/20 p-2">
                            <label className="mb-1 block text-xs font-bold uppercase tracking-[0.16em] text-amber-300/80">
                              Goethe Spielvorgabe
                            </label>
                            <input
                              inputMode="numeric"
                              disabled={!isAdminUnlocked}
                              value={
                                localHandicaps[goetheKey] ??
                                String(p.course_hcp_goethe ?? 0)
                              }
                              onChange={(e) => {
                                setAdminEditing(true);
                                setLocalHandicaps((current) => ({
                                  ...current,
                                  [goetheKey]: cleanNumericInput(e.target.value),
                                }));
                              }}
                              className="w-full rounded-2xl border border-amber-700/40 bg-stone-950 p-2.5 text-center text-amber-50 disabled:opacity-60"
                            />
                          </div>

                          <div className="rounded-xl border border-amber-700/20 bg-black/20 p-2">
                            <label className="mb-1 block text-xs font-bold uppercase tracking-[0.16em] text-amber-300/80">
                              Feininger Spielvorgabe
                            </label>
                            <input
                              inputMode="numeric"
                              disabled={!isAdminUnlocked}
                              value={
                                localHandicaps[feiningerKey] ??
                                String(p.course_hcp_feininger ?? 0)
                              }
                              onChange={(e) => {
                                setAdminEditing(true);
                                setLocalHandicaps((current) => ({
                                  ...current,
                                  [feiningerKey]: cleanNumericInput(e.target.value),
                                }));
                              }}
                              className="w-full rounded-2xl border border-amber-700/40 bg-stone-950 p-2.5 text-center text-amber-50 disabled:opacity-60"
                            />
                ...
                <Button
                  disabled={!isAdminUnlocked}
                  onClick={saveFullSetup}
                  className="mt-3 w-full rounded-2xl bg-amber-600 text-amber-50 disabled:opacity-50"
                >
                  {saving ? "Speichere ..." : "Admin-Einstellungen speichern"}
                </Button>
              </CardContent>
            </Card>
          </motion.section>
          ...
          <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }}>
            <Card className="mb-3 rounded-2xl border-amber-700/40 bg-[#20170f]/90 shadow-xl">
              <CardContent className="p-3">
                ...
                <p className="mt-1 text-sm text-amber-100/65">
                  Diese Einstellung wird nur lokal auf diesem Handy gespeichert.
                </p>

                <div className="mt-3 rounded-2xl border border-amber-700/30 bg-black/25 p-2.5">
                  <label className="mb-1 block text-sm text-amber-100/80">
                    Wer bin ich auf diesem Handy?
                  </label>
                  <select
                    value={myPlayerId}
                  ...
                  <p className="mt-2 text-xs text-amber-100/60">
                    Dieser Spieler wird auf diesem Handy beim Score-Zählen
                    ausgeblendet, damit man sich nicht selbst zählt.
                  </p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="mb-3 rounded-2xl border-amber-700/40 bg-[#20170f]/90 shadow-xl">
              <CardContent className="p-3">
                <div className="mb-3 rounded-xl border border-amber-700/30 bg-black/25 p-2.5">
                  <div className="text-xs uppercase tracking-[0.2em] text-amber-300/75">
                    Aktuell gespielt
                  </div>
                  <div className="mt-0.5 font-serif text-lg text-amber-200">
                    {displayedActiveRound?.round_name || "Runde 1"}
                  </div>
                  <div className="text-xs text-amber-100/65">
                    {activeCourse?.course_name || "Kein Kurs ausgewählt"}
                  </div>
                </div>
                ...
                <div className="mb-3 flex items-center justify-between">
                  <div>
                    ...
                    <h2 className="font-serif text-4xl font-black text-amber-200">
                      {activeHole}
                    </h2>
                  </div>

                  <div className="rounded-2xl border border-amber-700/50 bg-black/30 p-3 text-right text-sm text-amber-50">
                    <div className="text-amber-100">
                      Par <b className="text-amber-200">{activeHoleData.par}</b>
                    </div>
                    <div className="text-amber-100">
                      HCP <b className="text-amber-200">{activeHoleData.hcp}</b>
                    </div>
                    <div className="text-amber-100">{activeHoleData.meters} m</div>
                  </div>
                </div>
                ...
                {myCurrentStats ? (
                  <div className="mb-3 rounded-xl border border-amber-700/30 bg-black/25 p-2.5">
                    <div className="mb-2 flex items-center justify-between gap-2">
                      ...
                      <div className="font-serif text-sm text-amber-200">
                        {myCurrentStats.character_name}
                      </div>
                    </div>

                    <div className="grid grid-cols-2 gap-2 text-center text-sm">
                      <div className="rounded-xl bg-amber-50/5 p-2 text-amber-50">
                        <div className="text-amber-100">Schläge</div>
                        <b className="text-xl text-amber-200">
                          {myCurrentStats.played ? myCurrentStats.total : "–"}
                        </b>
                        <div className="mt-0.5 text-[11px] text-amber-100/70">
                          {myCurrentStats.played}/18 · Platz {myStrokeRank || "–"}
                        </div>
                      </div>
                      ...
                      <div className="rounded-xl bg-amber-50/5 p-2 text-amber-50">
                        ...
                        <b className="text-xl text-amber-200">
                          {myCurrentStats.netStableford}
                        </b>
                        <div className="mt-0.5 text-[11px] text-amber-100/70">
                          SpV {Number(myCurrentStats.course_hcp || 0)} · Platz{" "}
                          {myNetStablefordRank || "–"}
                        </div>
                      </div>

                      <div className="col-span-2 text-center text-[11px] text-amber-100/65">
                        Vorgabe Loch {activeHole}:{" "}
                        <b className="text-amber-200 tracking-[0.18em]">
                          {formatShotMarks(myShotsOnActiveHole)}
                        </b>
                      </div>
                    </div>
                  </div>
                ) : (
                  <div className="mb-3 rounded-xl border border-amber-700/30 bg-black/20 p-2.5 text-xs text-amber-100/75">
                    Unter Einstellungen kannst du festlegen, wer du bist. Danach
                    erscheint hier dein aktueller Score.
                  </div>
                )}
                ...
                <div className="rounded-2xl border border-amber-700/40 bg-amber-50/5 p-4">
                  <div className="mb-3 flex items-center justify-between gap-2">
                    <span className="font-serif text-lg text-amber-200">
                      {scoredPlayer?.character_name} · Loch {activeHole}
                    </span>
                    <span className="text-[11px] text-amber-100/65">
                      Vorgabe{" "}
                      <b className="text-amber-200 tracking-[0.18em]">
                        {formatShotMarks(scoredPlayerShotsOnActiveHole)}
                      </b>
                    </span>
                  </div>
                  ...
                  <div className="mb-3 grid grid-cols-6 gap-2">
                    {quickScores.map((value) => (
                      <button
                        key={value}
                        onClick={() => saveScore({ strokes: value, picked_up: false })}
                        className={cls(
                          "rounded-2xl border py-2.5 text-base font-bold",
                          Number(currentScore.strokes) === value &&
                            !normalizeBoolean(currentScore.picked_up)
                            ? "border-amber-300 bg-amber-500 text-amber-50"
                      ...
                      saveScore({
                        strokes:
                          cleanNumericInput(e.target.value) === ""
                            ? ""
                            : Number(cleanNumericInput(e.target.value)),
                        picked_up: false,
                      })
                    }
                    placeholder="z. B. 5"
                  ...
                  <div className="mb-3 rounded-2xl border border-amber-700/40 bg-black/25 p-2.5">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        ...
                        <div className="text-xs text-amber-100/65">
                          Wertet automatisch {pickedUpStrokes} Schläge und 0
                          Netto-Punkte.
                        </div>
                      </div>
                      <input
                        type="checkbox"
                        checked={normalizeBoolean(currentScore.picked_up)}
                        onChange={(e) =>
                          saveScore(
                            e.target.checked
                              ? { picked_up: true, strokes: pickedUpStrokes }
                              : { picked_up: false }
                          )
                        }
                  ...
                  <div className="mb-3 rounded-2xl border border-amber-700/40 bg-black/25 p-2.5">
                    <div className="mb-3 flex items-center justify-between">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          saveScore({
                            over_two_putts: e.target.checked,
                            putts_count: e.target.checked
                              ? currentScore.putts_count || 3
                              : "",
                          })
                        }
                        className="h-5 w-5 accent-amber-500"
                    ...
                    {normalizeBoolean(currentScore.over_two_putts) ? (
                      <div className="grid grid-cols-2 gap-2">
                        <button
                          type="button"
                          onClick={() =>
                            saveScore({ over_two_putts: true, putts_count: 3 })
                          }
                          className={cls(
                            "rounded-2xl border py-2.5 text-sm font-bold",
                            Number(currentScore.putts_count) === 3
                              ? "border-amber-300 bg-amber-500 text-amber-50"
                              : "border-amber-700/40 bg-stone-950 text-amber-100"
                          )}
                        >
                          3 Putt
                        </button>
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            saveScore({ over_two_putts: true, putts_count: 4 })
                          }
                          className={cls(
                            "rounded-2xl border py-2.5 text-sm font-bold",
                            Number(currentScore.putts_count) >= 4
                              ? "border-amber-300 bg-amber-500 text-amber-50"
                              : "border-amber-700/40 bg-stone-950 text-amber-100"
                          )}
                        >
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <Button
                      ...
                      className="rounded-2xl bg-stone-800 text-amber-100"
                    >
                      Zurück
                    </Button>
                    <Button
                      ...
                      className="rounded-2xl bg-amber-600 text-amber-50"
                    >
                      Nächstes Loch
                    </Button>
                  </div>
                  ...
                  <div className="mt-3 rounded-2xl border border-amber-700/30 bg-black/25 p-2.5">
                    ...
                    <select
                      ...
                      onChange={(e) => setScoredPlayerId(e.target.value)}
                      className="w-full rounded-2xl border border-amber-700/40 bg-stone-950 p-2.5 text-amber-50"
                    >
                  ...
                  currentScore.strokes != null ? (
                    <Button
                      onClick={saveRoundToArchive}
                      className="mt-2 w-full rounded-2xl bg-emerald-600 text-amber-50"
                    >
          ...
          <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }}>
            <Card className="mb-3 rounded-2xl border-amber-700/40 bg-black/35">
              <CardContent className="p-3 text-sm text-amber-100">
                <div className="text-xs uppercase tracking-[0.2em] text-amber-300/75">
                  Aktuell gespielt
                </div>
                <div className="mt-1 font-serif text-lg text-amber-200">
                  {displayedActiveRound?.round_name || "Runde 1"}
                </div>
                <div className="text-amber-100/65">
                  {activeCourse?.course_name || "Kein Kurs ausgewählt"}
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="mb-3 rounded-2xl border-amber-700/40 bg-[#20170f]/90 shadow-xl">
              <CardContent className="p-3">
                ...
                <LeaderboardTable
                  title="Klassisches Zählspiel"
                  players={strokePlayLeaderboard}
                  columns={[
                    {
                      label: "+/−",
                      render: (p) => formatToPar(p.toPar, p.played),
                      emphasize: true,
                    },
                    {
                      label: "Schläge",
                      render: (p) => (p.played ? p.total : "–"),
                    },
                    {
                      label: "Löcher",
                      render: (p) => String(p.played) + "/18",
                    },
                  ]}
                />
                ...
                <LeaderboardTable
                  ...
                  columns={[
                    {
                      label: "Punkte",
                      render: (p) => p.netStableford,
                      emphasize: true,
                    },
                    {
                      label: "SpV",
                      render: (p) => Number(p.course_hcp || 0),
                    },
                    {
                      label: "Löcher",
                      render: (p) => String(p.played) + "/18",
                ...
                <LeaderboardTable
                  ...
                  columns={[
                    {
                      label: "+/−",
                      render: (p) => formatToPar(p.hcpAdjustedToPar, p.played),
                      emphasize: true,
                    },
                    {
                      label: "Netto",
                      render: (p) => (p.played ? p.hcpAdjustedTotal : "–"),
                    },
                    {
                      label: "HCP",
                      render: (p) => p.hcpShotsUsed,
                    },
                    {
                      label: "Löcher",
                      render: (p) => String(p.played) + "/18",
                ...
                <LeaderboardTable
                  title="Brutto Punkte"
                  players={grossStablefordLeaderboard}
                  columns={[
                    {
                      label: "Punkte",
                      render: (p) => p.grossStableford,
                      emphasize: true,
                    },
                    {
                      label: "Schläge",
                ...
                <LeaderboardTable
                  title="Putt-Kasse"
                  players={puttPenaltyLeaderboard}
                  columns={[
                    {
                      label: "3 Putts",
                      render: (p) => `${p.threePutts} × 2 €`,
                    },
                    {
                      label: "4+ Putts",
                      render: (p) => `${p.fourPlusPutts} × 4 €`,
                    },
                    {
                      label: "Gesamt",
                      render: (p) => `${p.puttPenaltyEuro || 0} €`,
                      emphasize: true,
                    },
                  ]}
                />
