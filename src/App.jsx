...
function LordOfTheHolesApp() {
  ...
  const needsMyPlayerSelection = Boolean(identityFlowActive && (!myPlayerId || forceMyPlayerPromptOpen));
  const showDevicePlayerGate = Boolean(!showSplash && (!appLocked || lockAdminBypass) && (!myPlayerId || forceMyPlayerPromptOpen));
        ...
        <Card className="mb-2 rounded-2xl bg-transparent shadow-none">
          <CardContent className="p-2">
            ...
            {showFinalWinnerPopup ? (
              <div className="mb-2 rounded-3xl border border-amber-400/60 bg-stone-950/90 p-4 text-center text-amber-50 shadow-2xl shadow-black/60 backdrop-blur-sm">
                <div className="mx-auto mb-3 flex h-16 w-16 items-center justify-center rounded-full border border-amber-300/50 bg-black/25 text-3xl shadow-xl shadow-amber-950/40">♛</div>
                ...
                <button type="button" onClick={() => setWinnerPopupDismissedKey(finalWinnerPopupKey)} className="mt-3 w-full rounded-2xl border border-amber-500/45 bg-amber-600 px-4 py-2.5 text-sm font-bold text-amber-50">Krone anerkennen ×</button>
              </div>
              ...
              <div className="mb-2 rounded-3xl border border-amber-400/60 bg-stone-950/90 p-4 text-center text-amber-50 shadow-2xl shadow-black/60 backdrop-blur-sm">
                ...
                <button type="button" onClick={() => setRoundHonorDismissedKeys((current) => Array.from(new Set([...(current || []), displayedRoundHonorCelebration.key])))} className="mt-3 w-full rounded-2xl border border-amber-500/45 bg-amber-600 px-4 py-2.5 text-sm font-bold text-amber-50">{roundHonorCloseLabel}</button>
              </div>
            ) : (
              <>
            {myCurrentStats ? (
            ...
            <div className={cls("rounded-3xl border-2 p-2 transition-colors shadow-2xl", isScorerEntryMode ? "border-sky-300 bg-[linear-gradient(180deg,rgba(14,165,233,0.28),rgba(12,10,9,0.88))] ring-4 ring-sky-300/45 shadow-sky-950/60" : "border-amber-300 bg-[linear-gradient(180deg,rgba(245,158,11,0.30),rgba(12,10,9,0.88))] ring-4 ring-amber-300/40 shadow-amber-950/60", hasScoreMismatch ? "ring-red-500/55" : "")}>
              {myCurrentPlayer ? <><div className={cls("mb-2 rounded-2xl border-2 px-3 py-3 text-center text-sm font-black uppercase tracking-[0.16em] shadow-lg", isScorerEntryMode ? "border-sky-200 bg-sky-500 text-stone-950 shadow-sky-950/60" : "border-amber-200 bg-amber-500 text-stone-950 shadow-amber-950/60")}>{isScorerEntryMode ? `Mithril-Kontrolle · Dein eigener Score (${getPlayerLabel(myCurrentPlayer)})` : `Offizieller Score · Du zählst ${getPlayerLabel(scoredPlayer) || "einen Spieler"}`}</div><div className="mb-2 grid grid-cols-2 gap-2"><button type="button" onClick={() => setScoreEntryMode("player")} className={cls("rounded-2xl px-2 py-3 text-sm font-bold", !isScorerEntryMode ? "bg-amber-600 text-amber-50" : "bg-stone-800 text-amber-100", hasSelectedPlayerScoreMismatch && "ring-1 ring-red-400/60")}><span className="block truncate font-serif text-sm leading-none">{getPlayerLabel(scoredPlayer) || "Spieler"}</span>{hasSelectedPlayerScoreMismatch ? <span className="ml-1">⚠</span> : null}</button><button type="button" onClick={() => setScoreEntryMode("scorer")} className={cls("rounded-xl px-2 py-1.5 text-xs font-bold", isScorerEntryMode ? "bg-amber-600 text-amber-50" : "bg-stone-800 text-amber-100", hasOwnScoreMismatch && "ring-1 ring-red-400/60")}><span className="block truncate font-serif text-sm leading-none">Mein Score</span>{hasOwnScoreMismatch ? <span className="ml-1">⚠</span> : null}</button></div></> : null}
