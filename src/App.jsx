...
function LeaderboardTable({ title, players, columns }) {
  return (
    <div className="mb-3 overflow-hidden rounded-2xl border border-amber-700/30 bg-black/25">
      ...
      <div className="w-full overflow-hidden landscape:overflow-x-auto landscape:[scrollbar-width:none] landscape:[&::-webkit-scrollbar]:hidden">
        <table className="w-full table-fixed border-collapse text-xs text-amber-50 landscape:min-w-full landscape:text-[11px]">
          <thead>
            <tr className="text-left text-xs uppercase tracking-wider text-amber-100">
              <th className="px-2 py-1.5">#</th>
              <th className="px-2 py-1.5">Spieler</th>
              {columns.map((column, columnIndex) => <th key={column.label} className={cls("px-2 py-1.5 text-right", columnIndex > 1 && "hidden landscape:table-cell")}>{column.label}</th>)}
            </tr>
          </thead>
          <tbody>
            {players.map((p, index) => (
              <tr key={p.id} className="border-t border-amber-700/20">
                ...
                {columns.map((column, columnIndex) => <td key={column.label} className={cls("px-2 py-1.5 text-right", column.emphasize && "font-serif text-base text-amber-300 landscape:text-lg", columnIndex > 1 && "hidden landscape:table-cell")}>{column.render(p)}</td>)}
              </tr>
            ))}
          </tbody>
        </table>
...
function FunTable({ title, subtitle = "", players, columns, nameLabel = "Name" }) {
  ...
  return (
    <div className="mb-3 overflow-hidden rounded-2xl border border-amber-700/30 bg-black/20">
      ...
      <div className="w-full overflow-hidden landscape:overflow-x-auto landscape:[scrollbar-width:none] landscape:[&::-webkit-scrollbar]:hidden">
        <table className="w-full table-fixed border-collapse text-xs text-amber-50 landscape:min-w-full landscape:text-[11px]">
          <thead><tr className="text-left text-xs uppercase tracking-wider text-amber-100"><th className="px-2 py-1.5">Platz</th><th className="px-2 py-1.5">{hasHoleRows ? "Loch" : nameLabel}</th>{columns.map((column, columnIndex) => <th key={column.label} className={cls("px-2 py-1.5 text-right", columnIndex > 1 && "hidden landscape:table-cell")}>{column.label}</th>)}</tr></thead>
          <tbody>{players.map((item, index) => <tr key={item.id || `${item.course_id || "course"}-${item.hole_number}-${index}`} className="border-t border-amber-700/20"><td className="px-2 py-1.5 text-amber-200/75">{index + 1}</td><td className="px-2 py-1.5 font-semibold text-amber-100">{item.hole_number ? `${item.course_name || getCourseShortName(item.course_id)} · Loch ${item.hole_number}` : item.character_name || item.display_name || item.id}</td>{columns.map((column, columnIndex) => <td key={column.label} className={cls("px-2 py-1.5 text-right", column.emphasize && "font-serif text-base text-amber-300 landscape:text-lg", columnIndex > 1 && "hidden landscape:table-cell")}>{column.render(item, index)}</td>)}</tr>)}</tbody>
        </table>
      </div>
    </div>
  );
