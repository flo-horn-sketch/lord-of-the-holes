async function resetFlightDrawAfterFullReset() {
  ...
  function startFlightReveal(drawToReveal = null) {
    ...
    const hasExplicitServerDraw = Boolean(drawToReveal?.rounds?.length);

    if ((!hasExplicitServerDraw && !flightDrawLoadedFromServer) || !nextDraw?.rounds?.length) {
  ...
  function renderStartOverlay() {
    ...
    const showCeremony = Boolean(appLocked && flightRevealRunning && flightDraw?.rounds?.length && currentRevealRound);

    return (
      <div className="fixed inset-0 z-[100] bg-black text-amber-50">
        <div className="absolute inset-0 bg-cover bg-center bg-no-repeat" style={{ backgroundImage: "url('/lord-bg.webp')" }} />
        <div className="absolute inset-0 bg-black/35" />
        <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(0,0,0,0.08),rgba(0,0,0,0.54)_42%,rgba(0,0,0,0.86)_100%)]" />

        {!appLocked ? (
          <div className="absolute inset-x-0 bottom-8 flex justify-center px-6 pb-[env(safe-area-inset-bottom)]">
            <button type="button" disabled={splashEntering} onClick={enterRoundFromSplash} className="w-full max-w-xs rounded-2xl border border-amber-300/55 bg-black/60 px-5 py-2.5 font-serif text-lg font-black tracking-wide text-amber-200 shadow-2xl shadow-black/70 backdrop-blur-sm active:scale-[0.98] disabled:opacity-60">
          ...
          <div className="absolute inset-x-4 bottom-6 mx-auto max-w-sm pb-[env(safe-area-inset-bottom)]">
            <div className="rounded-3xl border border-amber-500/35 bg-black/60 p-3 text-center text-amber-50 shadow-2xl shadow-black/70 backdrop-blur-sm">
              <div className="text-[10px] uppercase tracking-[0.28em] text-amber-300/80">Lord of the Holes</div>
              <div className="mt-1 font-serif text-xl font-black text-amber-200">Der Rat ist noch nicht einberufen.</div>
              <div className="mt-1.5 text-xs leading-snug text-amber-100/80">Im Weimarer Land werden Stimmen gesenkt, alte Karten entrollt und verdächtig ernste Blicke ausgetauscht.</div>
              <div className="mt-3 rounded-2xl border border-amber-400/35 bg-amber-500/10 p-2 text-center">
                <div className="text-[10px] uppercase tracking-[0.24em] text-amber-300/80">Flight-Ziehung</div>
                <div className="mt-1 font-serif text-xl font-black leading-none text-amber-200">21.05.26</div>
                <div className="mt-0.5 font-serif text-lg font-black leading-none text-amber-100">20:00 Uhr</div>
              </div>
              <div className="mt-2 grid grid-cols-4 gap-1 rounded-2xl border border-amber-500/25 bg-black/35 p-1.5 text-center">
                <div><div className="font-serif text-lg font-black text-amber-200">{lockCountdown.days}</div><div className="text-[9px] uppercase tracking-[0.14em] text-amber-100/60">Tage</div></div>
                <div><div className="font-serif text-lg font-black text-amber-200">{String(lockCountdown.hours).padStart(2, "0")}</div><div className="text-[9px] uppercase tracking-[0.14em] text-amber-100/60">Std</div></div>
                <div><div className="font-serif text-lg font-black text-amber-200">{String(lockCountdown.minutes).padStart(2, "0")}</div><div className="text-[9px] uppercase tracking-[0.14em] text-amber-100/60">Min</div></div>
                <div><div className="font-serif text-lg font-black text-amber-200">{String(lockCountdown.seconds).padStart(2, "0")}</div><div className="text-[9px] uppercase tracking-[0.14em] text-amber-100/60">Sek</div></div>
              </div>
              <div className="mt-2 rounded-2xl border border-amber-500/25 bg-black/30 px-2.5 py-1.5 font-serif text-xs font-bold text-amber-100">Die Zeremonie startet nur mit gespeicherter Flight-Ziehung aus dem Admin-Bereich.</div>
            </div>
          </div>
        )}
        ...
        {appLocked && lockUnlockOpen ? (
          <div className="absolute inset-x-4 bottom-8 mx-auto max-w-xs rounded-2xl border border-amber-700/35 bg-black/75 p-3 text-amber-50 shadow-2xl shadow-black/70 backdrop-blur-sm">
            <div className="mb-2 text-xs uppercase tracking-[0.18em] text-amber-300/70">Admin</div>
            <input type="password" value={lockPasswordInput} onChange={(e) => setLockPasswordInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") enterLockedAppAsAdmin(); }} placeholder="Passwort" className="mb-2 w-full rounded-xl border border-amber-700/40 bg-stone-950 p-2 text-amber-50 placeholder:text-amber-100/30" autoFocus />
            <div className="grid grid-cols-2 gap-2">
              <button type="button" onClick={() => { setLockUnlockOpen(false); setLockPasswordInput(""); }} className="rounded-xl bg-stone-800 py-2 text-sm font-bold text-amber-100">Abbrechen</button>
              <button type="button" disabled={splashEntering || flightDrawSaving} onClick={enterLockedAppAsAdmin} className="rounded-xl bg-amber-600 py-2 text-sm font-bold text-amber-50 disabled:opacity-60">{splashEntering || flightDrawSaving ? "Öffne ..." : "Zeremonienmeister"}</button>
            </div>
            <button type="button" disabled={splashEntering || flightDrawSaving} onClick={enterAppWithoutFlightDraw} className="mt-2 w-full rounded-xl border border-amber-500/35 bg-stone-950/85 py-2 text-sm font-bold text-amber-100 disabled:opacity-60">Direkt in die App</button>
          </div>
        ) : null}
      </div>
    );
